...
optionally_change_class_name() {
  
  read -p "Do you want to change class name? (y/n): " CHOICE
  
  if [[ $CHOICE = 'Y' || $CHOICE = "y" ]] ; then
    
    read -p "New class name: " NEW_CLASS_NAME
    
    NEW_CLASS_NAME="${NEW_CLASS_NAME%.java}"
    
    if [[ ! -f "$DST/$NEW_CLASS_NAME" ]] ; then 
    
      OLD_CLASS_NAME_WITHOUT_JAVA_EXT="${SRC_FILENAME%.java}"
      
      
      mv "$DST/$SRC_FILENAME" "$DST/$NEW_CLASS_NAME.java"
      sed -i "s/$OLD_CLASS_NAME_WITHOUT_JAVA_EXT/$NEW_CLASS_NAME/g" "$DST/$NEW_CLASS_NAME.java"
