...
[ "$#" -lt 2 ] && { echo "usage: $0 PACKAGE_NAME CLASS_NAME [-s|--silent]" >&2; exit 1; }
...
if [[ -e "$PCKG_NAME/$CLASS_NAME" ]] ; then
  silent_echo "$SILENT_MODE" "Class $CLASS_NAME already exists"
  exit 1
fi
...
[ ! -f "$CONF_FILE" ] || 
[ ! -r "$CONF_FILE" ] && 
{ silent_echo "$SILENT_MODE" "Config file is not available" >&2 ; exit 2; }

GEN_MAIN=$(grep "MAIN=TRUE" "$CONF_FILE" > /dev/null && echo 'T' || echo 'F')
GEN_HELLO=$(grep "HELLO_WORLD=TRUE" "$CONF_FILE" > /dev/null && echo 'T' || echo 'F')

[ "$GEN_MAIN" = 'T' ] && silent_echo "$SILENT_MODE" "Generator will create main function"
[ "$GEN_MAIN" = 'T' -a "$GEN_HELLO" = 'T' ] && silent_echo "$SILENT_MODE" "Generator will create hello world code"
